...
tsv.list1 <- lapply(tsv.locs, function(x){
  t1 <- read_tsv(x) %>%
    separate(target_id, into = letters[1:3]) %>%                                 # separate target_id to three cols
...
write_csv(final.df, "../../data_frames/kallisto_results.csv")
...
We know which genes were not present at this step. These were genes with **any** deletion, i.e. pseudogenized by small deletion that caused a frameshift, or or completely absent. Functionally, this is somewhat identical. Make a data frame that contains the genes with deletions in each line, to be used later. 
```{r}
