...
my.mutations <- read_csv("../../data_frames/table_s8_mutations.csv")
...
The TFs
```{r}
genes <- c("flhC", "flhD", "argR", "iclR", "cueR", "cysB", "gcvA", "purR", "arcB", "cusR")
...
pc.bottom <- (gene.plots$iclR | gene.plots$arcB) + plot_layout(widths = c(.7, .3))
...
```{r}
pd.top <- heatmap_function(cu, title = "Copper/silver transport", threshold = 3, fig_letter = "D", x_labs = cu.x.labs)

pd.bottom <- gene.plots$cueR | gene.plots$cusR

pd <- (pd.top / pd.bottom) + plot_layout(heights = c(.7, .3))
```
### Panel E

Sulfur compounds
```{r}
sulfur <- c("cysB", "cysP", "cysU", "cysW", "cysA", "cysM", "tauA", "tauB", "tauC", "tauD","cysD", "cysN", "cysC", "cysH", "cysI", "cysJ")

s.x.labs <- c(expression(underline("cysB"), "cysP", "cysU", "cysW", "cysA", "cysM", "tauA", "tauB", "tauC", "tauD","cysD", "cysN", "cysC", "cysH", "cysI", "cysJ"))

names(sulfur) <- c("sa", rep("sb", 5), rep("sc", 4), rep("se", 3), rep("sf", 3))
```

```{r}
pe.top <- heatmap_function(sulfur, title = "Sulfur compounds", threshold = 3, fig_letter = "E", x_labs = s.x.labs)

pe <- (pe.top / gene.plots$cysB) + plot_layout(heights = c(.7, .3))
```
...
```{r}
pf.top <- heatmap_function(x = gly, title = "Glycine cleavage", threshold = 3, fig_letter = "F", x_labs = gly.x.labs)

pf <- (pf.top / (gene.plots$purR | gene.plots$gcvA)) + plot_layout(heights = c(.7, .3))
```
...
(ff <- (top / middle / bottom) + plot_layout(guides = "collect"))
```

```{r}
ggsave(ff, filename = "../../figures/h_fig_s11.pdf", width = 15, height = 15, device = cairo_pdf)

ggsave(ff, filename = "../../figures/h_fig_s11.png", width = 15, height = 15, dpi = 300)
```