...
The volume formula is $V = \frac{\pi}{4} * W^2 * (L - \frac{W}{3})$ where V = volume, W = width, L = length. As code, it's `volume = (pi/4)*(W^2)*(L-(W/3))`. Also add columns for mutator status and age.
```{r}
...
```{r}
png.b <- rasterGrob(readPNG("../../figures/b_bac_images2.png"), interpolate = TRUE)
...
t-tests to show that those are indeed different
```{r}
combo.df %>% 
  filter(!(grepl("REL", line))) %>% 
  compare_means(ratio~comp, method = "t.test", group.by = "line", data = .)
```
...
pf <- fdf %>% 
  ggplot(., aes(median_volume, rna, label = line, color = mutator))+
  geom_smooth(method = "lm", inherit.aes = FALSE, aes(x = median_volume, y = rna), se = FALSE, color = "black")+
  geom_point(size = 2.5)+
  geom_text_repel(min.segment.length = .00001, box.padding = .5, alpha = .5, seed = 1)+
  stat_cor(aes(label = ..r.label.., x = median_volume, y = rna), inherit.aes = FALSE)+
  theme(panel.background = element_blank(),
        axis.line = element_line(color = "black"),
        text = element_text(size = 13))+
