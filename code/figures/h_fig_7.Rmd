...
```{r}
# the list of mutations, and the specific clones for oru mutations
...
pb.top <- heatmap_function(nad, title = "NAD biosynthesis", threshold = 3, fig_letter = "B", x_labs = nad.x.labs)
...
(ff <- (pa | pb) + plot_layout(guides = "collect", widths = c(.6, .4)))
```


```{r}
ggsave(ff, filename = "../../figures/h_fig_7.pdf", width = 12, height = 5, device = cairo_pdf)

ggsave(ff, filename = "../../figures/h_fig_7.png", width = 12, height = 5, dpi = 300)
```
