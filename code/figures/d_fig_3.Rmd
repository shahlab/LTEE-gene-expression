...
```{r}
adf <- my.mutations %>% 
  filter(type == "SNP" & !grepl("intergenic", gene_position) & mutator_status == "non-mutator") %>% 
  group_by(gene_name, population, mutator_status) %>% # collapse multiple mutations in same gene in one line to one row
  tally() %>% 
  ungroup() %>% 
...
pa <- adf %>% 
  ...
  scale_x_continuous(limits = c(.5,6.5), breaks = 1:6)+
  theme_classic()+
  theme(text = element_text(size = 13))+
  labs(x = "Number of non-mutator lines\nin which a gene has a SNP",
       y = "Count",
       title = "A")+
  scale_y_continuous()
