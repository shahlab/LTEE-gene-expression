...
```{r}
evo.kdf <- read_csv("../../data_frames/kallisto_results.csv") %>%                  # read the df in
...
```{r}
anc.reps <- read_csv("../../data_frames/kallisto_results.csv") %>%
...
```{r}
anc.means <- read_csv("../../data_frames/kallisto_results.csv") %>%
  filter(!(grepl("Ara", line))) %>%
  select(-length, -eff_length) %>% 
...
```{r}
deseqdf <- read_csv("../../data_frames/deseq_results.csv") %>% 
...
```{r}
rrdf <- read_csv("../../data_frames/riborex_results.csv") %>% 
